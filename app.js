...
// ── Command Center: left sidebar nav (desktop-wide only, >=1180px, see the
// matching @media block in styles.css) ────────────────────────────────────
// Real destinations: data-gamepick-pane items reuse the exact same
// window.DiamondNavigateToPane routing the top nav/mobile drawer already
// use. data-sidebar-href items link straight to a real existing page.
// data-sidebar-action items are one-off real behaviors already possible on
// this page (go home, scroll to an already-rendered hub section).
// data-sidebar-soon items (Players, Weather Center, Injuries, Betting
// Tools, Watchlist, Alerts) are honest "not built yet" placeholders --
// there's no aggregated page or feed behind any of those yet (per-player
// signals exist for some of them, e.g. watchlist stars and roster status,
// but no cross-board page assembling them) -- so they show a toast instead
// of a silently-broken link or a page with nothing real on it.
(function(){
  function showToast(msg){
    var el = document.getElementById('dr-toast');
    if (!el) {
    ...
    clearTimeout(el._drToastTimer);
    el._drToastTimer = setTimeout(function(){ el.classList.remove('show'); }, 2400);
  }

  function syncActive(pane){
    document.querySelectorAll('.dr-sidebar-tab').forEach(function(btn){
      var isPane = btn.hasAttribute('data-gamepick-pane');
      btn.classList.toggle('active', isPane && btn.getAttribute('data-gamepick-pane') === pane);
    });
  }
  function syncActiveHome(){
    document.querySelectorAll('.dr-sidebar-tab').forEach(function(btn){
      btn.classList.toggle('active', btn.getAttribute('data-sidebar-action') === 'home');
    });
  }

  function goHomeAnd(after){
    try { window.location.hash = ''; } catch(e) {}
    syncActiveHome();
    // Clearing the hash only fires 'hashchange' if it actually changes --
    // dispatch it manually so an already-home click still re-confirms the
    // hub is showing, same as every other hash-based route on this page.
    window.dispatchEvent(new Event('hashchange'));
    if (after) setTimeout(after, 80);
  }

  function bind(){
    var sidebar = document.getElementById('dr-sidebar');
    if (!sidebar) return;
    ...
    sidebar.querySelectorAll('.dr-sidebar-tab[data-gamepick-pane]').forEach(function(btn){
      ...
      btn.addEventListener('click', function(){
        var pane = btn.getAttribute('data-gamepick-pane');
        syncActive(pane);
        if (typeof window.DiamondNavigateToPane === 'function') window.DiamondNavigateToPane(pane, { noScroll: true });
      });
    });

    sidebar.querySelectorAll('.dr-sidebar-tab[data-sidebar-href]').forEach(function(btn){
      if (btn.dataset.drSidebarReady) return;
      btn.dataset.drSidebarReady = '1';
      btn.addEventListener('click', function(){
        window.location.href = btn.getAttribute('data-sidebar-href');
      });
    });

    sidebar.querySelectorAll('.dr-sidebar-tab[data-sidebar-action]').forEach(function(btn){
      if (btn.dataset.drSidebarReady) return;
      btn.dataset.drSidebarReady = '1';
      ...
      btn.addEventListener('click', function(){
        if (action === 'home') {
          goHomeAnd();
        } else if (action === 'projections' || action === 'news') {
          goHomeAnd(function(){
            var targetId = action === 'news' ? 'dr-hub-news' : 'dr-hub-projections';
            var target = document.getElementById(targetId);
            var sideContent = document.getElementById('dr-hub-side-content');
            var sideToggle = document.getElementById('dr-hub-side-toggle');
            if (sideContent && sideToggle && sideToggle.offsetParent && !sideContent.classList.contains('expanded')) {
              sideToggle.click();
            }
    ...
    sidebar.querySelectorAll('.dr-sidebar-tab[data-sidebar-soon]').forEach(function(btn){
      if (btn.dataset.drSidebarReady) return;
      btn.dataset.drSidebarReady = '1';
      btn.addEventListener('click', function(){
        showToast(btn.getAttribute('data-sidebar-soon') + ' is coming soon.');
      });
    });

    var activePane = document.querySelector('#props .gamepick-pane.active');
    if (/^#?gamepick=/.test(window.location.hash || '') && activePane) syncActive(activePane.getAttribute('data-gamepick-pane'));
    else syncActiveHome();

    window.addEventListener('hashchange', function(){
      setTimeout(function(){
        if (/^#?gamepick=/.test(window.location.hash || '')) {
          var p = document.querySelector('#props .gamepick-pane.active');
          if (p) syncActive(p.getAttribute('data-gamepick-pane'));
        } else {
          syncActiveHome();
        }
      }, 30);
  ...
  // Read the header's real rendered height (varies by breakpoint/content,
  // same reasoning navigateToPane's own scroll-offset calc already uses)
  // into a CSS var so the sidebar's fixed `top` always clears it exactly.
  function syncHeaderHeightVar(){
    var headerEl = document.querySelector('.header');
    if (headerEl) document.documentElement.style.setProperty('--dr-header-h', headerEl.getBoundingClientRect().height + 'px');
  }
