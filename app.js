...
// Fixed categorical pitch-type -> color order (dataviz-skill-validated for
// dark-surface CVD safety at #06060f, the site's actual --bg -- all 8 slots
// clear lightness band / chroma floor / CVD adjacent-pair / normal-vision /
// contrast checks). A pitch keeps the same color across every chart in the
// Pitcher Analytics Dashboard ("color follows the entity, never its rank");
// a pitch type outside this list (rare -- knuckleballs, eephus, etc.) falls
// back to a neutral gray rather than inventing an unvalidated 9th hue.
...
const PITCH_TYPE_FALLBACK_COLOR = '#8fa3c8';
function pitchTypeColor(name) { return PITCH_TYPE_COLORS[name] || PITCH_TYPE_FALLBACK_COLOR; }

// Expected strikeouts / expected innings for a starter's projected outing --
// shared with the K Props board's own per-pitcher projection (see buildRow in
// the K Props IIFE), same shape/constants, pulled into one place so both
// surfaces run literally the same model instead of two copies that could
// drift apart. projIP only needs innings/games-started; projK also needs the
// pitcher's (recent-form-blended) K/9 and the opposing lineup's real K rate.
function pitcherExpectedKAndIP(k9, ip, gs, oppKpct) {
...
// Real "is this batter actually in today's lineup" badge for board rows (HR
// Threats, Hits/RBI/TB/SB/H+R+RBI) -- reads the same repo-synced lineups.json
// getRepoLineupForGame already serves the pitcher-matchup panel from, no new
// fetch. Batter's own side is derivable from teamAbbr vs the game's homeAbbr
// already present on every board row. Returns '' (badge hidden) when there's
// no real lineup data yet, rather than guessing.
function lineupBadgeHTML(gamePk, teamAbbr, homeAbbr) {
  if (gamePk == null || !teamAbbr) return '';
  const side = teamAbbr === homeAbbr ? 'home' : 'away';
  const entry = getRepoLineupForGame(gamePk, side);
  if (!entry || !entry.lineup || !entry.lineup.length) return '';
  return entry.confirmed
    ? '<span class="dr-row-lineup-badge confirmed">✅ Lineup Confirmed</span>'
    : '<span class="dr-row-lineup-badge expected">🔮 Expected Lineup</span>';
}
...
// ── Pitcher Analytics Dashboard (roadmap 7.1) ───────────────────────────
// Organizes the pitcher pop-out into five named sections instead of one long
// stack, per the roadmap's explicit ask -- Arsenal (pitcherArsenalPanelHTML,
// unchanged entry point), Performance (real L10/L15/L20 form + today's
// expected K/IP), Matchup (the existing real opposing-lineup panel, plus a
// real lineup-strength read once it loads), Environment (real park factor +
// live weather for today's specific game), Support (his own team's real
// bullpen fatigue tier). Every section identifies its own period/source in
// its subtext per the acceptance criteria, and degrades to an honest empty
// state -- never a fabricated number -- when a pitcher's situation (opener,
...
const PITCHER_DASHBOARD_SECTION_META = {
  ...
  environment: { icon: '🌤️', title: 'ENVIRONMENT' },
  support:     { icon: '🛟', title: 'SUPPORT' },
};
function pitcherDashboardSectionWrap(key, contentEl, subtext) {
  const meta = PITCHER_DASHBOARD_SECTION_META[key];
  const wrap = document.createElement('div');
  wrap.className = 'dr-pdash-section';
  wrap.setAttribute('data-pdash-section', key);
  const head = document.createElement('div');
  head.className = 'dr-pdash-section-head';
  head.innerHTML = `<span class="dr-pdash-section-icon">${meta.icon}</span><span class="dr-pdash-section-title">${meta.title}</span>` +
    (subtext ? `<span class="dr-pdash-section-sub" id="pdash-sub-${key}-${contentEl.id || ''}">${drEscAttr(subtext)}</span>` : '');
  wrap.appendChild(head);
  wrap.appendChild(contentEl);
  return wrap;
}

// Opposing lineup's average real season K rate, read from whatever's already
// in lineupCache at call time (the lineup fetch this same modal-open kicks
// off) rather than a second network round trip. If the lineup hasn't
// resolved yet, falls back to the league-average 22% -- same fallback the K
// Props board's own boxscore-based version already uses when data isn't
// available -- and gets one real refresh below once the lineup lands.
function opponentKPctForPid(pid, meta) {
  const cacheKey = `${meta.gamePk}-${meta.side}`;
  const lineup = lineupCache[cacheKey]?.lineup || [];
  const kpcts = lineup.map(b => {
    const s = b.stats || {};
    const so = parseInt(s.strikeOuts), pa = parseInt(s.plateAppearances);
    return (so > 0 && pa > 0) ? so / pa : null;
  }).filter(v => v != null);
  if (!kpcts.length) return 0.22;
  return kpcts.reduce((a, b) => a + b, 0) / kpcts.length;
}

// L10/L15/L20 rate-stat trend, reusing the exact recentPitchingForm() the K
// Props/Diamond Report Pick models already call at N=5 -- just called at
// three more window sizes here, cached per (pid, N) for 5 minutes so
// re-opening the same pitcher's modal doesn't re-fetch. A pitcher with fewer
// starts than a given window (rookie, midseason call-up) shows "–" for that
// column rather than a fabricated number. Season baseline is read straight
// out of prRows (already fetched for the table this modal was opened from),
// no extra request.
async function renderPitcherPerformanceSection(pid, meta, wrap) {
  const row = prRows.find(r => normalizePitcherId(r.pitcher.id) === pid) || null;
  const windows = [10, 15, 20];
  const forms = await Promise.all(windows.map(w => recentPitchingForm(pid, w).catch(() => null)));
  if (!wrap.isConnected) return; // modal closed/reopened for a different pitcher while these were in flight
  const seasonEra = row?.era, seasonWhip = row?.whip, seasonK9 = row?.rawK9;

  if (!forms.some(Boolean) && seasonEra == null) {
    wrap.innerHTML = `<div class="mu-empty" style="color:var(--muted)">No recent-start data available yet for ${drEscAttr(meta.pitcherName || 'this pitcher')}.</div>`;
    return;
  }

  function trendCell(val, seasonVal, dec, lowerIsBetter) {
    if (val == null) return `<td class="num"><span style="color:var(--muted)">–</span></td>`;
    let style = '';
    if (seasonVal != null) {
      const delta = val - seasonVal;
      const meaningfulThreshold = dec === 1 ? 0.3 : 0.15; // K/9 noise floor is coarser than ERA/WHIP's
      if (Math.abs(delta) >= meaningfulThreshold) {
        const better = lowerIsBetter ? delta < 0 : delta > 0;
        style = ` style="color:${better ? 'var(--green)' : '#f4a261'};font-weight:700"`;
      }
    }
    return `<td class="num"${style}>${val.toFixed(dec)}</td>`;
  }

  const rateRows = [
    { label: 'ERA',  dec: 2, lowerIsBetter: true,  season: seasonEra,  vals: forms.map(f => f?.era) },
    { label: 'WHIP', dec: 2, lowerIsBetter: true,  season: seasonWhip, vals: forms.map(f => f?.whip) },
    { label: 'K/9',  dec: 1, lowerIsBetter: false, season: seasonK9,   vals: forms.map(f => f?.k9) },
  ];
  const tableRows = rateRows.map(r => `<tr><td><strong>${r.label}</strong></td><td class="num">${r.season != null ? r.season.toFixed(r.dec) : '–'}</td>${windows.map((w, i) => trendCell(r.vals[i], r.season, r.dec, r.lowerIsBetter)).join('')}</tr>`).join('');
  const startsFound = forms.map((f, i) => f ? `L${windows[i]} (${f.starts})` : null).filter(Boolean).join(', ');

  const oppKpct = opponentKPctForPid(pid, meta);
  const expected = row ? pitcherExpectedKAndIP(seasonK9 ?? 8, row.rawIp ?? 0, row.gamesStarted ?? 1, oppKpct) : null;

  wrap.innerHTML = `
    <div class="dr1041-table-wrap"><table class="dr1041-pitch-table">
      <thead><tr><th>Rate</th><th>2026 Season</th><th>Last 10</th><th>Last 15</th><th>Last 20</th></tr></thead>
      <tbody>${tableRows}</tbody>
    </table></div>
    <div class="zone-note" style="margin-top:8px">${startsFound ? `Real starts logged per window: ${startsFound}. ` : ''}Green = trending better than his season baseline over that window; orange = trending worse; muted/– = not enough recent starts logged yet for that window.</div>
    ${expected ? `<div class="dr-pdash-expected"><div class="dr-pdash-expected-stat"><strong>${expected.projIP.toFixed(1)}</strong><span>Expected IP</span></div><div class="dr-pdash-expected-stat"><strong>${expected.projK.toFixed(1)}</strong><span>Expected K</span></div><span class="zone-note">Today's projection vs ${meta.oppAbbr || 'the opponent'} — same model as the K Props board.</span></div>` : ''}
  `;
}

async function renderPitcherEnvironmentSection(meta, wrap) {
  await loadParkFactors().catch(() => {});
  ...
  const parkRow = parkIdx != null
    ...
    : `<div class="dr-pdash-env-row"><span class="dr-pdash-env-label">Park HR Factor</span><span class="dr-pdash-env-value" style="color:var(--muted)">Not available for ${drEscAttr(homeAbbr || 'this park')}</span></div>`;
  const weatherRow = weather
    ? `<div class="dr-pdash-env-row"><span class="dr-pdash-env-label">Conditions</span><span class="dr-pdash-env-value">${weather.temp}°F · wind ${weather.wind} mph${weather.precip > 0 ? ' · rain' : ''}</span></div>`
    : `<div class="dr-pdash-env-row"><span class="dr-pdash-env-label">Conditions</span><span class="dr-pdash-env-value" style="color:var(--muted)">${homeAbbr ? 'Dome / indoor park' : 'Not available yet'}</span></div>`;

  wrap.innerHTML = `<div class="dr-pdash-env">${parkRow}${weatherRow}</div><div class="zone-note" style="margin-top:8px">Today's game at ${drEscAttr(homeAbbr || '–')} — real park HR factor + live conditions (Open-Meteo), refreshed every 15 minutes.</div>`;
}

async function renderPitcherSupportSection(meta, wrap) {
...
function pollForLineupThenRefreshMatchup(pid, meta, matchupSubEl, performanceWrap, matchupTableWrap, driversWrap) {
  ...
  (function tick() {
    ...
    if (lineup && lineup.length) {
      const opsVals = lineup.map(b => parseFloat(b.stats?.ops)).filter(v => Number.isFinite(v) && v > 0);
      if (opsVals.length && matchupSubEl) {
        const avgOps = opsVals.reduce((a, b) => a + b, 0) / opsVals.length;
        const tier = avgOps >= .760 ? 'Tough' : avgOps <= .690 ? 'Favorable' : 'Average';
        const color = tier === 'Tough' ? '#f4a261' : tier === 'Favorable' ? 'var(--green)' : 'var(--text)';
...
// row calls this directly; renderPRTable calls it again after every refresh
// so an open modal survives the live-score re-render instead of going
// stale.
function openPitcherLineupModal(pidRaw) {
  ...
  body.appendChild(pitcherDashboardSectionWrap('arsenal', arsenalWrap, `${(meta.pitcherName || '').split(' ').pop()}'s repertoire — real Statcast pitch-level data, 2026 season`));

  const performanceWrap = document.createElement('div');
  performanceWrap.id = `pr-performance-${pid}`;
  performanceWrap.innerHTML = `<div style="padding:10px 0;color:var(--muted);font-size:12px"><span class="spin"></span> Loading recent form…</div>`;
  body.appendChild(pitcherDashboardSectionWrap('performance', performanceWrap, 'Real last 10/15/20 starts vs season baseline (MLB Stats API game logs)'));
  ...
  const environmentWrap = document.createElement('div');
  environmentWrap.id = `pr-environment-${pid}`;
  environmentWrap.innerHTML = `<div style="padding:10px 0;color:var(--muted);font-size:12px"><span class="spin"></span> Loading park &amp; weather…</div>`;
  body.appendChild(pitcherDashboardSectionWrap('environment', environmentWrap, "Today's ballpark + live conditions"));

  const supportWrap = document.createElement('div');
  supportWrap.id = `pr-support-${pid}`;
  supportWrap.innerHTML = `<div style="padding:10px 0;color:var(--muted);font-size:12px"><span class="spin"></span> Loading bullpen support…</div>`;
  body.appendChild(pitcherDashboardSectionWrap('support', supportWrap, `${meta.teamAbbr || 'His'} bullpen — real season relief numbers`));
  ...
  renderPitcherSupportSection(meta, supportWrap);
...
const ZONE_LABELS = ['In/High','High','Out/High','Inside','Middle','Away','In/Low','Low','Out/Low'];
// Simple horizontal bar chart shared by the Pitcher Analytics Dashboard's
// Usage/Velocity/Whiff charts -- one row per pitch type, bar color keyed to
// PITCH_TYPE_COLORS so a pitch reads as the same color across every chart in
// this panel ("color follows the entity"). Every bar carries a direct value
// label (not just a color-coded legend), since the 8-slot palette validated
// in the 6-8 CVD floor band, which is only legal with that secondary
// encoding. `domainMax` is a fixed scale, not each chart's own data max, so
// bars stay honestly comparable rather than reading off a rescaled axis.
function pitchBarChartHTML(title, subtitle, rows, domainMax, fmt) {
  if (!rows.length) return '';
  const bars = rows.map(r => {
    const pct = Math.max(0, Math.min(100, (r.value / domainMax) * 100));
    const color = pitchTypeColor(r.name);
    return `<div class="dr-pchart-row" title="${drEscAttr(r.name)}: ${drEscAttr(fmt(r.value))}">
      <span class="dr-pchart-label"><span class="dr-pchart-swatch" style="background:${color}"></span>${drEscAttr(r.name)}</span>
      <div class="dr-pchart-track"><div class="dr-pchart-fill" style="background:${color}" data-fill="${pct.toFixed(1)}%"></div></div>
      <span class="dr-pchart-value">${fmt(r.value)}${r.badge || ''}</span>
    </div>`;
  }).join('');
  return `<div class="dr-pchart">
...
function pitcherArsenalPanelHTML(pitcherId, pitcherName) {
  ...
  const hasCareerPitchMix = !!(profile?.career?.byPitch?.length);
  const hasRollingData = !!(rollingProfile?.byPitch?.length);
  ...
  // veloTxt intentionally dropped from every pitch-table row here (it was
  // always empty anyway -- data/pitcher-statcast.json's byPitch has no
  // avgVelo field; only data/pitcher-rolling.json does) now that velocity
  // gets its own real chart below, keyed off the data that actually has it.
  function buildPitchRows(byPitchArr) {
    return (byPitchArr || []).map(p => {
      ...
      return pitchEffRow(p.name, p.usagePct, avg, woba, slg, hr, whiffPct, '');
    }).join('');
  }
  const pitchRows = hasRealPitchMix ? buildPitchRows(profile.byPitch) : '';
  const careerPitchRows = hasCareerPitchMix ? buildPitchRows(profile.career.byPitch) : '';

  // ── Real charts: Pitch Usage, Velocity, Whiff Rate ──────────────────────
  // Usage/whiff come from the season Statcast profile (broadly synced, 700+
  // pitchers); velocity comes from data/pitcher-rolling.json, which only
  // covers today's probable starters -- but that's exactly this dashboard's
  // whole audience, so it's not a real coverage gap in practice. Velocity's
  // delta badge is colored from the PITCHER's own perspective (this is his
  // scouting report, not a batter's): velocity trending UP over his last 3
  // starts is good for him (green), trending down is a real concern
  // (orange) -- the opposite framing from the batter-facing Batter vs
  // Pitcher modal's rollingDeltaTag, which colors the same numbers from the
  // hitter's point of view.
  let chartsHTML = '';
  if (hasRealPitchMix || hasRollingData) {
    const usageRows = hasRealPitchMix ? profile.byPitch.filter(p => p.usagePct != null).map(p => ({ name: p.name, value: p.usagePct })) : [];
    const whiffRows = hasRealPitchMix ? profile.byPitch.filter(p => p.whiffPct != null).map(p => ({ name: p.name, value: p.whiffPct })) : [];
    let veloRows = [];
    if (hasRollingData) {
      veloRows = rollingProfile.byPitch.filter(p => p.seasonVelo != null).map(p => {
        const delta = p.veloDelta;
        const badge = delta != null && Math.abs(delta) >= 0.5
          ? ` <span style="color:${delta > 0 ? 'var(--green)' : '#f4a261'};font-weight:700">(${delta > 0 ? '+' : ''}${delta.toFixed(1)} L3)</span>`
          : '';
        return { name: p.name, value: p.seasonVelo, badge };
      });
    }
    const usageChart = pitchBarChartHTML('Pitch Usage', '2026 season', usageRows, 100, v => v.toFixed(0) + '%');
    const veloChart = pitchBarChartHTML('Velocity', 'season, vs last 3 starts', veloRows, 105, v => v.toFixed(1) + ' mph');
    const whiffChart = pitchBarChartHTML('Whiff Rate', '2026 season', whiffRows, 100, v => v.toFixed(0) + '%');
    if (usageChart || veloChart || whiffChart) chartsHTML = `<div class="dr-pchart-grid">${usageChart}${veloChart}${whiffChart}</div>`;
  }

  function rollingDeltaTagPitcherView(delta, noiseThreshold) {
    if (delta == null) return `<span style="color:var(--muted)">–</span>`;
    // Pitcher-centric coloring (see chartsHTML comment above): up is good here.
  ...
  let rollingRows = '';
  if (hasRollingData) {
    rollingRows = rollingProfile.byPitch.map(p => `
      <tr>
  ...
  const rollingLastDates = (rollingProfile?.lastStartDates || []).join(', ');
  ...
  const pstatsSubtext = pstatsModes.length > 1
    ? `Real synced pitch-level data for ${pitcherName} — toggle ${pstatsModes.map(pstatsModeLabel).join(' / ')}. Every metric below is 2026 season unless the L3 Trend tab is active.`
    : hasRealPitchMix
      ? `Real synced pitch-level data for ${pitcherName}. 2026 season.`
      : hasRollingData
  ...
  return chartsHTML + pitchEffectivenessTableHTML + attackZoneHTML;
}

// Real per-matchup scouting report for HR Threats cards, at the user's request to
// replace the generic templated "grades at X% because..." summary with the same
// "SCOUTING REPORT — HOW TO HIT A HOME RUN" vulnerability bullets already shown in
// the pop-out matchup modal (openMatchup/renderMatchupModal) — built from the same
// three live MLB Stats API calls (batter season stats, pitcher season stats, career
// head-to-head) so a card's report and the modal's report always agree.
const hrScoutingReportCache = new Map();
function hrScoutingVulnsHTML(batterName, pitcherName, bs, ps, h2h) {
  function fv(v, dec=3) {
  ...
  const pHR9 = fv(ps.homeRunsPer9,2), pAVG = fv(ps.avg), pWHIP = fv(ps.whip,2);
  const bISO = (bs.slg&&bs.avg) ? fv((parseFloat(bs.slg)-parseFloat(bs.avg)).toFixed(3)) : '–';
  const h2hAB = fi(h2h.atBats), h2hAVG = fv(h2h.avg), h2hK = fi(h2h.strikeOuts);
  ...
  if (hasH2H && parseInt(h2h.homeRuns) > 0)
    vulns.push({ icon:'💥', text: `Career H2H: ${batterName} has hit <strong>${fi(h2h.homeRuns)} HR</strong> off ${pitcherName} in ${h2hAB} AB. Historical edge for the batter.` });
  else if (hasH2H)
  ...
  return vulns.map(v => `<div class="vuln-item"><span class="vuln-icon">${v.icon}</span><span style="color:var(--text)">${v.text}</span></div>`).join('');
}
async function loadHRScoutingReport(batterId, batterName, pitcherId, pitcherName) {
  const key = `${batterId}:${pitcherId}`;
  if (hrScoutingReportCache.has(key)) return hrScoutingReportCache.get(key);
  const promise = (async () => {
    try {
      ...
      return hrScoutingVulnsHTML(batterName, pitcherName, bs, ps, h2h) ||
        '<div class="vuln-item"><span style="color:var(--muted)">No scouting signal cleared threshold for this matchup yet.</span></div>';
    } catch(e) {
      return '<div class="vuln-item"><span style="color:var(--muted)">Scouting report unavailable right now.</span></div>';
    }
  })();
  hrScoutingReportCache.set(key, promise);
  return promise;
}
...
// to the final value. Double rAF (not single) so the 0% paint is guaranteed
// to land before the flip -- a single rAF can get coalesced into the same
// paint as the initial render on some browsers, which would skip the
// animation entirely. Purely a CSS transition under the hood, so it already
// goes near-instant automatically for prefers-reduced-motion users via the
// blanket rule in styles.css -- no separate reduced-motion branch needed
// here.
function drFillMeters(root) {
  if (!root) return;
  requestAnimationFrame(function(){
    requestAnimationFrame(function(){
      root.querySelectorAll('.dr1027-meter-fill[data-fill], .dr-pchart-fill[data-fill]').forEach(function(f){
...
function renderMatchupModal(body, { batterName, pitcherName, batterId, pitcherId, batterPerson={}, pitcherPerson={}, batterSplits=[], pitcherSplits=[], h2h, bs, ps, bx, px, hotHitter, pitcherProfile, nearHRList=null, recentHRList=null, hrSprayList=null, todayParkAbbr=null, rollingProfile=null, countPerfProfile=null, situationalProfile=null, bullpenInfo=null, parkHrIndex=null }) {
  ...
  // Raw 0-1 normalized value array (or null) for a zone map -- shared by the individual
  // pitcher/batter grid cell-builder below AND the combined-grid cell-builder, which
  // needs the raw numbers (not pre-built HTML) to blend both sides' values per cell.
  function zoneValsFromMap(zMap, valueFn) {
    if (!zMap) return null;
    const vals = [1,2,3,4,5,6,7,8,9].map(z => {
      const cell = zMap[z];
      if (!cell) return null;
  ...
  function zoneCellsFromMap(zMap, valueFn, titleSuffix) {
    const vals = zoneValsFromMap(zMap, valueFn);
    if (!vals) return null;
    return vals.map((v, i) => {
  ...
  if (attackZonePitches.length) {
    ...
    <div class="zone-section" id="${auid}" data-attack-zone-toggle>
      ...
      <div class="zone-wrap">
        ...
        <div>
          <div class="dr1042-split-toggle" role="tablist" aria-label="Attack zone pitch toggle" style="flex-wrap:wrap;height:auto">${tabs}</div>
          <div class="zone-legend" style="margin-top:10px">
            <div class="zone-leg-item"><div class="zone-leg-swatch" style="background:#4a1010"></div><span style="color:#ff6b6b">Heavy usage (20%+) — his primary spot for this pitch</span></div>
            <div class="zone-leg-item"><div class="zone-leg-swatch" style="background:#3a2010"></div><span style="color:#f4a261">Frequent (14–19%)</span></div>
            <div class="zone-leg-item"><div class="zone-leg-swatch" style="background:#1a2a10"></div><span style="color:#90ee60">Occasional (8–13%)</span></div>
            <div class="zone-leg-item"><div class="zone-leg-swatch" style="background:#0d1a0d"></div><span style="color:#3a6a3a">Rare (&lt;8%)</span></div>
          </div>
          <div class="zone-note" style="margin-top:10px;max-width:220px">Where ${pitcherName.split(' ').pop()} actually locates each individual pitch — his real location tendency pitch by pitch, not just the blended zone profile above. Tap (or hover) any cell for its exact %.</div>
        </div>
      </div>
      </div>
    </div>`;
...
// The Strike Zone Matchup / Attack Zone by Pitch / HR Zones grids already carry a
// per-cell description via the native title attribute (color-coded cells) or an SVG
// <title> child (HR Zones' scatter dots) -- both work as a hover tooltip on desktop,
// but neither has any mobile-tap equivalent (a phone has no "hover" state at all), so
// a mobile user could see the color-coding but never actually read what a given cell
// or dot means. Reuses the exact same title text (no separate copy to keep in sync)
// in a small floating popover on tap, auto-dismissing after a few seconds or on the
// next tap elsewhere -- desktop hover behavior is completely untouched.
document.addEventListener('click', function(e) {
  const existing = document.getElementById('dr-viz-tooltip');
  if (existing) existing.remove();
  const target = e.target.closest && e.target.closest('.zone-section .sz-cell[title], .zone-section circle');
  if (!target) return;
  const text = target.getAttribute('title') || (target.querySelector('title') ? target.querySelector('title').textContent : '');
  if (!text) return;
  const tip = document.createElement('div');
  tip.id = 'dr-viz-tooltip';
  tip.className = 'dr-viz-tooltip';
...
// ── STANDARDIZED FILTER BAR (Roadmap 2.3) ───────────────────────────────
// One shared filter system reused across every projection board (HR
// Threats, K Props, the 5 Prop Intelligence boards) so the same filter
// categories/labels/interaction pattern show up everywhere instead of each
// board hand-rolling its own. Boards that are genuinely "related views" of
// the same underlying player pool (the 5 Prop Intelligence boards) pass the
// SAME groupKey so a team/hand/etc. chosen on one stays applied when the
// user switches to another; boards with a different row shape (HR Threats,
// K Props) get their own independent groupKey. Declared top-level (like
// drShowError/drSkeletonRowsHTML above) so every IIFE elsewhere in the file
// can call it too.
window.__drStdFilters = window.__drStdFilters || {};
function drDefaultFilterState() {
  return { team: '', pos: '', pitcherHand: '', batterHand: '', gamePk: '', stadium: '', weather: '' };
}
function drFilterState(key) {
  if (!window.__drStdFilters[key]) window.__drStdFilters[key] = drDefaultFilterState();
  return window.__drStdFilters[key];
}
function drSetFilter(key, field, value, renderFn) {
  drFilterState(key)[field] = value || '';
  if (typeof renderFn === 'function') renderFn();
}
function drClearAllFilters(key, boardKey, renderFn) {
  window.__drStdFilters[key] = drDefaultFilterState();
  if (boardKey && window.__drBoardSearch) window.__drBoardSearch[boardKey] = '';
  if (typeof renderFn === 'function') renderFn();
}
function drActiveFilterCount(key, boardKey) {
  const f = drFilterState(key);
  let count = Object.keys(f).filter(k => !!f[k]).length;
  if (boardKey && window.__drBoardSearch && (window.__drBoardSearch[boardKey] || '').trim()) count++;
  return count;
}
// A real, already-computed HR weather multiplier (see loadHRPotential's
// weatherHRMult, itself airDensityHRMult * windHRMult -- both capped at
// +/-15-20%) bucketed into 3 tiers for filtering. Never a fabricated
// qualitative label -- just 3 real bands over a number the site's own HR
// model already uses.
function drWeatherTier(mult) {
  if (mult == null || !Number.isFinite(mult)) return null;
  if (mult >= 1.04) return 'boost';
  if (mult <= 0.96) return 'suppressed';
  return 'neutral';
}
function drWeatherTierLabel(tier) {
  return tier === 'boost' ? 'HR Boost' : tier === 'suppressed' ? 'HR Suppressed' : tier === 'neutral' ? 'Neutral' : '–';
}
// Real distinct values present in the CURRENT (pre-filter) row set for a
// given field -- so a dropdown never offers a choice that can't possibly
// match anything today (e.g. a team not playing), while still reflecting
// every real value actually on the board, not a fixed hardcoded list.
function drDistinctFilterOptions(rows, field, mapLabel, sortByLabel = true) {
  const seen = new Set();
  const out = [];
  (rows || []).forEach(r => {
    const v = r ? r[field] : null;
    if (v == null || v === '' || v === '–' || seen.has(v)) return;
    seen.add(v);
    out.push({ value: String(v), label: mapLabel ? mapLabel(v, r) : String(v) });
  });
  if (sortByLabel) out.sort((a, b) => String(a.label).localeCompare(String(b.label)));
  return out;
}
function drApplyStandardFilters(key, rows) {
  const f = drFilterState(key);
  if (!Object.keys(f).some(k => f[k])) return rows;
  return (rows || []).filter(r => {
    if (f.team && r.teamAbbr !== f.team) return false;
    if (f.pos && r.pos !== f.pos) return false;
    if (f.pitcherHand && r.pitcherHand !== f.pitcherHand) return false;
    if (f.batterHand && r.hand !== f.batterHand) return false;
    if (f.gamePk && String(r.gamePk || '') !== f.gamePk) return false;
    if (f.stadium && r.homeAbbr !== f.stadium) return false;
    if (f.weather && drWeatherTier(r.weatherHRMult) !== f.weather) return false;
    return true;
  });
}
// Renders the actual filter controls -- opts: { renderFnName (required, the
// board's own render function name as a string, called on every change),
// hasPosition, hasPitcherHand, hasBatterHand, hasWeather (booleans; Team/
// Game Time/Stadium always show since every projection row has a real game).
// allRows should be the board's full, PRE-filter row set so option lists
// always reflect every real value on the board today.
function drStandardFilterBarHTML(key, boardKey, allRows, opts) {
  opts = opts || {};
  const f = drFilterState(key);
  const count = drActiveFilterCount(key, boardKey);
  const onChange = (field) => `drSetFilter('${key}','${field}',this.value,${opts.renderFnName})`;
  const selectHTML = (field, label, options, allLabel) => {
    const optsHTML = `<option value="">${drEscAttr(allLabel || ('All ' + label))}</option>` +
      options.map(o => `<option value="${drEscAttr(o.value)}"${f[field] === o.value ? ' selected' : ''}>${drEscAttr(o.label)}</option>`).join('');
    return `<label class="dr-filter-select-wrap"><span class="dr-filter-label">${drEscAttr(label)}</span><select class="dr-filter-select" onchange="${onChange(field)}">${optsHTML}</select></label>`;
  };

  const parts = [];
  parts.push(selectHTML('team', 'Team', drDistinctFilterOptions(allRows, 'teamAbbr')));
  if (opts.hasPosition) parts.push(selectHTML('pos', 'Position', drDistinctFilterOptions(allRows, 'pos')));
  if (opts.hasPitcherHand) parts.push(selectHTML('pitcherHand', 'Pitcher Throws', [{ value: 'R', label: 'RHP' }, { value: 'L', label: 'LHP' }], 'All Pitchers'));
  if (opts.hasBatterHand) parts.push(selectHTML('batterHand', 'Batter Bats', [{ value: 'R', label: 'RHB' }, { value: 'L', label: 'LHB' }, { value: 'S', label: 'Switch' }], 'All Batters'));

  const gameOptsRaw = drDistinctFilterOptions(allRows, 'gamePk', (pk, r) => {
    const t = r.gameTimestamp ? new Date(r.gameTimestamp).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZone: 'America/Chicago' }) : '';
    return `${t ? t + ' · ' : ''}${r.teamAbbr || '?'} vs ${r.oppAbbr || '?'}`;
  }, false);
  // Real chronological order (first pitch time), not alphabetical -- matches
  // every other "Game" selector already on the site.
  const gameTsByPk = {};
  (allRows || []).forEach(r => { if (r.gamePk != null && gameTsByPk[r.gamePk] == null) gameTsByPk[r.gamePk] = r.gameTimestamp || 0; });
  gameOptsRaw.sort((a, b) => (gameTsByPk[a.value] || 0) - (gameTsByPk[b.value] || 0));
  parts.push(selectHTML('gamePk', 'Game Time', gameOptsRaw, 'All Games'));

  parts.push(selectHTML('stadium', 'Stadium', drDistinctFilterOptions(allRows, 'homeAbbr', abbr => stadiumCoords[abbr]?.name || abbr)));
  if (opts.hasWeather) parts.push(selectHTML('weather', 'Weather', [{ value: 'boost', label: 'HR Boost' }, { value: 'neutral', label: 'Neutral' }, { value: 'suppressed', label: 'HR Suppressed' }], 'All Conditions'));

  const clearBtn = count > 0
    ? `<button type="button" class="dr-filter-clear-all" onclick="drClearAllFilters('${key}','${boardKey || ''}',${opts.renderFnName})">Clear all filters (${count})</button>`
    : '';
  const badgeHTML = `<span class="dr-filter-count-badge${count > 0 ? ' active' : ''}">${count}</span>`;

  return `<div class="dr-filter-bar" data-filter-key="${key}">
    <button type="button" class="dr-filter-toggle-btn" onclick="this.closest('.dr-filter-bar').classList.toggle('dr-filter-bar-open')">⚙ Filters ${badgeHTML}</button>
    <div class="dr-filter-controls">${parts.join('')}${clearBtn}</div>
  </div>`;
}
// Shared "nothing matched" message for a filtered-to-zero board, with a
// real one-click way out (calls the exact same drClearAllFilters used by
// the bar's own Clear button) rather than leaving the user staring at a
// blank board with no path back to the full list.
function drFilterEmptyStateHTML(key, boardKey, renderFnName, boardLabel) {
  return `<div class="mu-empty" style="padding:24px;text-align:center">No ${drEscAttr(boardLabel || 'players')} match the selected filters.
    <button type="button" class="dr-filter-clear-all" style="margin:10px auto 0;display:inline-block" onclick="drClearAllFilters('${key}','${boardKey || ''}',${renderFnName})">Clear all filters</button>
  </div>`;
}
...
function renderKProps() {
  ...
  // Standardized cross-site filters (Team/Pitcher Throws/Game Time/Stadium) --
  // option lists build off the full kPropsData set (not gameFilteredProps) so
  // they never shrink as other filters narrow the visible rows.
  const stdFilterBar = drStandardFilterBarHTML('k', 'k', kPropsData, { hasPitcherHand: true, renderFnName: 'renderKProps' });
  gameFilteredProps = drApplyStandardFilters('k', gameFilteredProps);
  ...
  const noKMatches = !sortedProps.length && ((window.__drBoardSearch.k || '').trim() || drActiveFilterCount('k', 'k') > 0);
  ...
  ${noKMatches ? drFilterEmptyStateHTML('k', 'k', 'renderKProps', 'pitchers') : noKLiveGames ? `<div class="mu-empty" style="padding:24px">No games are live right now. Turn off LIVE ONLY to see the full board.</div>` : `<div style="overflow-x:auto;overscroll-behavior-x:contain;touch-action:pan-y;min-width:0">
    ${sortedProps.map(p => {
      ...
      return `<div class="dr109-card${alreadyHit ? ' prop-hit' : missedK ? ' prop-miss' : ''}" style="cursor:pointer" onclick="if(!event.target.closest('button,a'))window.openKPropLineupModal('${p.pitcherId}','${p.pitcherName.replace(/'/g,"\\'")}','${p.teamAbbr}','${p.oppAbbr}')">
        ...
        <div class="dr109-card-head">
          <div class="dr109-player">
            ...
            <div style="min-width:0">
              <div class="dr109-name">${p.pitcherName}</div>
              ${alreadyHit ? '<div class="dr109-badge-row"><span class="prop-hit-badge">✓ Projection Hit</span></div>' : missedK ? '<div class="dr109-badge-row"><span class="prop-miss-badge">✗ Missed</span></div>' : ''}
...
async function openKPropLineupModal(pitcherId, pitcherName, teamAbbr, oppAbbr) {
  ...
  Promise.all([loadPitcherStatcast(), loadPitcherRolling()]).then(() => {
    ...
    if (arsenalWrap) { arsenalWrap.innerHTML = pitcherArsenalPanelHTML(pid, pitcherName); drFillMeters(arsenalWrap); }
  });
...
(function(){
  ...
  async function seasonPitching(pid){
    try {
      ...
      var stat = (d && d.people && d.people[0] && d.people[0].stats && d.people[0].stats[0] && d.people[0].stats[0].splits && d.people[0].stats[0].splits[0] && d.people[0].stats[0].splits[0].stat) || {};
      // pitchHand is a top-level field this same call already returns (no extra
      // fetch/hydrate needed) -- stashed on the stat object so buildRow below can
      // read it without a second request, same convention loadHRPotential uses.
      try { stat.__pitchHand = (typeof handCode === 'function') ? handCode(d.people[0], 'pitchHand') : null; } catch(e) {}
      return stat;
    } catch(e){ return {}; }
  }
  ...
  async function buildRow(g, side, stat){
    ...
    var projIP = pitcherExpectedKAndIP(k9, ip, gs, 0.22).projIP;
    ...
    var projK = pitcherExpectedKAndIP(k9, ip, gs, oppKpct).projK;
    var sbLine = null;
    ...
    var propRow = {
      ...
      teamAbbr: g.teams[side].team.abbreviation,
      oppAbbr: g.teams[opp].team.abbreviation,
      // Real, already-available (no extra fetch) -- pitcherHand comes from
      // seasonPitching's stashed stat.__pitchHand; homeAbbr resolves the game's
      // actual home team regardless of which side this pitcher is on, powering
      // the standardized Stadium/Weather filters.
      pitcherHand: stat.__pitchHand || null,
      homeAbbr: side === 'home' ? g.teams[side].team.abbreviation : g.teams[opp].team.abbreviation,
...
(function(){
  ...
  // Game Center: promotes what used to be a single-pane Team Performance modal
  // into a tabbed one, reusing the exact tab-bar pattern already built for the
  // player Matchup modal (data-matchup-tabs / .mu-tabs / .mu-tab-btn /
  // .mu-tab-pane -- see the delegated click listener elsewhere in this file
  // that already switches any container matching that pattern, so no new JS
  // wiring is needed here beyond emitting the right markup). Deliberately kept
  // as an enriched modal rather than a new hash-routed page -- the nav/routing
  // code elsewhere in this file is fragile and heavily patched, and a modal
  // gets the same "open a Game Center from any game card" outcome without
  // touching it. Team Performance tab is untouched logic, just retargeted at
  // its own pane id instead of the whole modal body.
  function gameCenterLineupsHTML(gamePk, awayAbbr, homeAbbr, awayPitcherName, homePitcherName){
    if (gamePk == null) return '<div class="mu-empty">Lineups not available for this game.</div>';
    function sideHTML(side, teamAbbr, pitcherName){
      var entry = getRepoLineupForGame(gamePk, side);
      var lineup = (entry && entry.lineup) || [];
      var badge = lineup.length
        ? (entry.confirmed ? '<span class="dr-row-lineup-badge confirmed">✅ Lineup Confirmed</span>' : '<span class="dr-row-lineup-badge expected">🔮 Expected Lineup</span>')
        : '<span class="dr-row-lineup-badge">Lineup not yet posted</span>';
      var rows = lineup.map(function(b, i){
        return '<div class="gc-lineup-row"><span class="gc-lineup-slot">'+(i+1)+'</span><span class="gc-lineup-name">'+esc(b.name||'')+'</span><span class="gc-lineup-pos">'+esc(b.pos||'')+'</span></div>';
      }).join('') || '<div class="mu-empty" style="padding:10px 0;text-align:left">No lineup posted yet.</div>';
      return '<div class="gc-lineup-side"><h4>'+esc(teamAbbr)+' '+badge+'</h4>'+(pitcherName?'<div class="gc-lineup-pitcher">SP: '+esc(pitcherName)+'</div>':'')+rows+'</div>';
    }
    return '<div class="gc-lineups-grid">'+sideHTML('away',awayAbbr,awayPitcherName)+sideHTML('home',homeAbbr,homePitcherName)+'</div>';
  }
  function gameCenterWinProbHTML(gamePk, awayAbbr, homeAbbr){
    var model = gamePk != null ? window.drWinProbStore && window.drWinProbStore[gamePk] : null;
    if (!model) return '<div class="mu-empty">Win probability not available for this game yet.</div>';
    var aAbbr2 = model.awayAbbr || awayAbbr, bAbbr2 = model.homeAbbr || homeAbbr;
    function barRow(abbr, pct){
      var isWinner = model.winnerAbbr === abbr;
      var color = isWinner ? 'var(--green)' : 'var(--muted)';
      return '<div class="gc-winprob-row"><span>'+esc(abbr)+'</span><div class="gc-winprob-track"><div class="gc-winprob-fill" style="width:'+pct+'%;background:'+color+'"></div></div><span>'+pct+'%</span></div>';
    }
    return '<div class="gc-winprob">'+barRow(aAbbr2,model.awayPct)+barRow(bAbbr2,model.homePct)+(model.confidence?'<div class="gc-winprob-conf">Confidence: '+esc(model.confidence)+'</div>':'')+'</div>';
  }
  window.gcOpenHeadline=function(hash, playerName){
    closeTeamMatchup();
    if (!hash) return;
    window.location.hash = hash.replace(/^#/,'');
    if (playerName && hash === '#gamepick=hr') {
      setTimeout(function(){ if (typeof window.setPropSearchFilter === 'function') window.setPropSearchFilter('hr', playerName); }, 60);
    }
  };
  function gameCenterNewsHTML(related){
    if (!related.length) return '<div class="mu-empty">No related stories for this game yet.</div>';
    return related.map(function(h){
      var hash = (h.link && h.link.hash) || '';
      var pname = (h.link && h.link.playerName) || '';
      return '<button type="button" class="gc-news-row" onclick="window.gcOpenHeadline(\''+esc(hash)+'\',\''+esc(pname).replace(/'/g,"\\'")+'\')">'
        + '<div class="gc-news-title">'+esc(h.title||'')+'</div><div class="gc-news-blurb">'+esc(h.blurb||'')+'</div>'
        + '</button>';
    }).join('');
  }

  async function openTeamMatchup(aid,bid,aName,bName,aAbbr,bAbbr,gamePk,awayPitcherId,awayPitcherName,homePitcherId,homePitcherName){
    ...
    if(sub)sub.textContent='Game Center';
    body.innerHTML =
      '<div data-matchup-tabs>'
      + '<div class="mu-tabs" role="tablist" aria-label="Game Center sections">'
        + '<button type="button" class="mu-tab-btn active" data-tab="gc-tp" role="tab" aria-selected="true">⚖️ Team Performance</button>'
        + '<button type="button" class="mu-tab-btn" data-tab="gc-lineups" role="tab" aria-selected="false">🧢 Lineups</button>'
        + '<button type="button" class="mu-tab-btn" data-tab="gc-winprob" role="tab" aria-selected="false">📈 Win Probability</button>'
        + '<button type="button" class="mu-tab-btn" data-tab="gc-news" role="tab" aria-selected="false">📰 News</button>'
      + '</div>'
      + '<div class="mu-tab-pane active" data-tab="gc-tp" id="gc-tp-pane"><div class="mu-empty" style="padding:20px 0;text-align:center"><span class="spin"></span> Loading team performance…</div></div>'
      + '<div class="mu-tab-pane" data-tab="gc-lineups" id="gc-lineups-pane"><div class="mu-empty" style="padding:20px 0;text-align:center"><span class="spin"></span> Loading lineups…</div></div>'
      + '<div class="mu-tab-pane" data-tab="gc-winprob" id="gc-winprob-pane">'+gameCenterWinProbHTML(gamePk,aAbbr,bAbbr)+'</div>'
      + '<div class="mu-tab-pane" data-tab="gc-news" id="gc-news-pane"><div class="mu-empty" style="padding:20px 0;text-align:center"><span class="spin"></span> Loading related stories…</div></div>'
      + '</div>';
    overlay.style.display='flex';
    document.body.style.overflow='hidden';

    (async function(){
      var pane=document.getElementById('gc-tp-pane'); if(!pane) return;
      try{
        await tpBase(false);
        ...
        pane.innerHTML='<div class="tp-note"><strong>Team Performance:</strong> compares current-season head-to-head results and season records.</div>'+tpCard(a,b,aStats,bStats,hStats);
      }catch(e){
        pane.innerHTML='<div class="mu-empty" style="color:var(--accent)">Could not load team performance right now.</div>';
      }
    })();

    (async function(){
      var pane=document.getElementById('gc-lineups-pane'); if(!pane) return;
      try{
        await loadRepoLineups();
        pane.innerHTML = gameCenterLineupsHTML(gamePk,aAbbr,bAbbr,awayPitcherName,homePitcherName);
      }catch(e){
        pane.innerHTML='<div class="mu-empty">Lineups not available for this game yet.</div>';
      }
    })();

    // News: a real join on player id (pitchers + both lineups' batters), since
    // headline objects carry no gamePk/team field to filter on directly.
    (async function(){
      var pane=document.getElementById('gc-news-pane'); if(!pane) return;
      try{
        var data = await drFetchDailyJSON('data/daily-headlines.json');
        var headlines = (data && data.headlines) || [];
        var ids = {};
        if (awayPitcherId != null) ids[String(awayPitcherId)] = 1;
        if (homePitcherId != null) ids[String(homePitcherId)] = 1;
        await loadRepoLineups();
        ['away','home'].forEach(function(side){
          var entry = gamePk != null ? getRepoLineupForGame(gamePk, side) : null;
          ((entry && entry.lineup) || []).forEach(function(b){ if (b.id != null) ids[String(b.id)] = 1; });
        });
        var related = headlines.filter(function(h){ return h.link && h.link.playerId != null && ids[String(h.link.playerId)]; });
        pane.innerHTML = gameCenterNewsHTML(related);
      }catch(e){
        pane.innerHTML='<div class="mu-empty">Could not load related stories.</div>';
      }
    })();
  }
...
(function(){
  ...
  // only reach a bare identifier that's actually a window property. This is
...
(function(){
  ...
  // and filters, just flips which side of isHit('hr',r) gets shown. Persisted on
  // window (not a local var) so it survives the page's periodic full re-renders the
  // same way the existing filter Set does.
  window.toggleHRPCompletedView = function(){
    window.__hrpShowCompletedOnly = !window.__hrpShowCompletedOnly;
    renderHRPTableV1032();
  };
  ...
  // ── Player Trend Indicators (roadmap 5.2) ──────────────────────────────
  // Standardized, documented Hot Streak/Trending Up/Cooling Off/Cold labels, computed
  // the same way for every player -- a separate, additive signal from isOnFire/isDue/
  // isDrought/isFavorable above, which are deliberately left untouched since they're
  // already captured on graded HR Threats picks server-side (update-tracker.mjs) for
  // the calibration report; changing their definitions here would silently invalidate
  // that tracked history.
  //
  // Rule (last 10 games vs season rate, since last10HR is already on every row with no
  // extra fetch -- "such as the last 5, 10, or 15 games" in the spec, picking the one
  // period this board already has real data for):
  //   expectedHRPer10 = this player's season HR rate, projected over ~40 AB (a rough
  //   10-games'-worth estimate) -- same reasoning MC_AB_PER_PA-style rate conversions
  //   use elsewhere in this codebase, just simplified to an AB count instead of PA.
  //   ratio = actual last10HR / expectedHRPer10 (floored so a near-zero expectation
  //   doesn't blow the ratio up to a meaningless number).
  //   ratio >= 1.75 (and at least 2 real last-10 HRs, not just 1 lucky one) -> Hot Streak
  //   ratio >= 1.15 -> Trending Up
  //   0 last-10 HR while the season rate expected at least 1 -> Cold
  //   ratio <= 0.5 -> Cooling Off
  //   otherwise -> no indicator (genuinely neutral, not worth a badge)
  // Below a 40-AB season sample, the same read is still shown but flagged as a small
  // sample rather than suppressed outright, per the "small samples are clearly marked"
  // acceptance criterion -- suppressing it would just look like the indicator silently
  // doesn't exist for that player, which is worse than an honest caveat.
  function computePlayerTrendIndicator(r){
    var last10HR = dr113Last10HRValue(r);
    if (last10HR == null) return null;
    last10HR = n(last10HR);
    var ab = n(r.atBats);
    var hrSeason = n(r.hrSeason);
    if (ab <= 0) return null;
    var expectedHRPer10 = (hrSeason / ab) * 40;
    var ratio = last10HR / Math.max(expectedHRPer10, 0.4);
    var smallSample = ab < 40;
    var label = null, tone = '';
    if (last10HR >= 2 && ratio >= 1.75) { label = 'Hot Streak'; tone = 'red'; }
    else if (ratio >= 1.15) { label = 'Trending Up'; tone = 'green'; }
    else if (last10HR === 0 && expectedHRPer10 >= 1.0) { label = 'Cold'; tone = ''; }
    else if (ratio <= 0.5) { label = 'Cooling Off'; tone = ''; }
    if (!label) return null;
  ...
  function playerTrendIndicatorChip(r){
    ...
    if (!t) return '';
    var icon = t.label === 'Hot Streak' ? '📈' : t.label === 'Trending Up' ? '↗️' : t.label === 'Cooling Off' ? '↘️' : '🧊';
    return labelChip(icon + ' ' + t.label.toUpperCase(), t.period + (t.smallSample ? ' · small sample' : ''), t.tone);
  }

  // ── Featured Home Run Pick (roadmap 5.1) ──────────────────────────────
  // A flagship card above the regular list, always the single highest-hrProb
  // player in the CURRENT (live, already-filtered-to-not-final) row array --
  // recomputed fresh on every render rather than cached/frozen, so a player
  // who gets scratched after being the top pick simply drops out on the next
  // refresh the same way they'd drop out of the regular list, with no extra
  // "is this player scratched" check needed.
  function pickFeaturedHRRow(arr){
    if (!arr || !arr.length) return null;
    return arr.reduce(function(best, cur){ return n(cur.hrProb) > n(best.hrProb) ? cur : best; }, arr[0]);
  }
  function buildFeaturedHRPickHTML(r){
    if (!r) return '';
    var p = n(r.hrProb);
    var l10 = dr113Last10HRValue(r);
    var bpPct = ballparkPalFactorForPlayer(r.gamePk, r.id);
    // statcastHotHitters is keyed by both id and lowercased name (see loadStatcastHotHitters) --
    // same dual-key lookup every other consumer of this store already uses.
    var hh = (typeof statcastHotHitters !== 'undefined' && statcastHotHitters) ? (statcastHotHitters[String(r.id)] || statcastHotHitters[String(r.name||'').toLowerCase()]) : null;
    var trendBadges = [];
    if (r.isOnFire) trendBadges.push(labelChip('🔥 ON FIRE', Math.round(n(r.onFireScore)), 'red'));
    if (r.isDue) trendBadges.push(labelChip('⚡ DUE', 'YES', 'gold'));
    if (r.isDrought) trendBadges.push(labelChip('❄️ DROUGHT', 'YES', 'red'));
    ...
    var statChips = [
      ...
      hrChip('Ballpark Factor', bpPct != null ? (bpPct>0?'+':'')+bpPct+'%' : '–', bpPct>0?'green':bpPct<0?'red':''),
      hrChip('Last 10 HR', l10==null?'–':l10, n(l10)>=2?'green':''),
    ].filter(Boolean);
    var uid = 'featured-hrp-' + r.id + '-' + (r.pitcherId||0);
    return '<div class="dr1027-hr-card dr1027-featured-pick" style="cursor:pointer" onclick="if(!event.target.closest(\'button,a\')){openMatchup('+JSON.stringify(String(r.id))+','+JSON.stringify(r.name||'')+','+JSON.stringify(String(r.pitcherId||''))+','+JSON.stringify(r.pitcherName||'')+');}">'
      + '<div class="dr1027-featured-badge">💥 FEATURED HOME RUN PICK</div>'
      + '<div class="dr1027-hr-head">'
        + '<img class="dr1027-hr-photo" loading="lazy" decoding="async" src="'+hs(r.id)+'" onerror="this.style.visibility=\'hidden\'" alt="">'
        + '<div><div class="dr1027-hr-name">'+esc(r.name||'–')+'</div>'
        + '<div class="dr1027-hr-meta">'+esc(r.teamAbbr||'–')+' · '+esc(r.pos||'–')+' · vs '+esc(r.oppAbbr||'–')+(r.pitcherName?' · '+esc(r.pitcherName):'')+'</div></div>'
        + '<div class="dr1027-hr-score"><strong>'+p.toFixed(1)+'%</strong><span>HR Probability</span><em>GRADE '+grade(p)+'</em></div>'
      + '</div>'
      + '<div class="dr1027-chip-row">'+trendBadges.concat(statChips).join('')+'</div>'
      + '<div class="dr1027-why" id="'+uid+'-why">'+(r.pitcherId?'<span class="spin"></span> Loading scouting report…':'<span style="color:var(--muted)">No opposing pitcher assigned yet — scouting report unavailable.</span>')+'</div>'
      + '<div class="dr1027-featured-weather" id="'+uid+'-weather" style="font-size:11px;color:var(--muted);margin-top:6px">Checking game weather…</div>'
      + '<div style="font-size:10px;color:var(--muted);margin-top:8px;text-align:right">Updated '+new Date().toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})+'</div>'
      + '</div>';
  }
  // Async-fills the AI explanation + weather blurb on the featured card after
  // the main render, same "placeholder now, fill in when ready" pattern every
  // regular card's own .dr1027-why already uses (loadHRScoutingReport is a
  // shared cache, so this doesn't double-fetch if the same matchup already
  // has a pending/resolved request from the regular card list).
  function fillFeaturedHRPickAsync(r){
    if (!r) return;
    var uid = 'featured-hrp-' + r.id + '-' + (r.pitcherId||0);
    if (r.pitcherId) {
      loadHRScoutingReport(r.id, r.name, r.pitcherId, r.pitcherName).then(function(html){
        var t = document.getElementById(uid+'-why'); if (t) t.innerHTML = html;
      });
    }
    var wEl = document.getElementById(uid+'-weather');
    if (wEl && r.teamAbbr && r.oppAbbr) {
