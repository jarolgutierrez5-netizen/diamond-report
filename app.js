...
(function(){
  ...
  // HR pick tiers (roadmap: codebase audit) -- a real, non-overlapping
  // classification built entirely from signals this board already computes
  // for every row, no new modeling. Elite/High Confidence reuse the exact
  // same 25%/20% score cutoffs already established and validated elsewhere
  // in this codebase (confidenceTier() in generate-performance-report.mjs,
  // the same buckets analyze-hr-matchups.mjs's calibration report uses) --
  // not new numbers invented for this feature. Value is the one genuinely
  // new definition here: a real corroborating signal (favorable matchup,
  // strong Matchup Edge, a real hot streak, or real warning-track power)
  // on a player whose raw predicted probability alone wouldn't otherwise
  // stand out -- exactly the "worth a second look" a Value tier implies.
  // Longshot is everything else that still cleared this board's own real
  // inclusion bar. The separate Featured Player hero card above this list
  // already covers the single best pick of the day -- this only tiers the
  // list underneath it, so "Featured" is never duplicated as a section.
  function hrTierFor(r) {
    var p = n(r.hrProb);
    if (p >= 25) return 'elite';
    if (p >= 20) return 'highConfidence';
    var hasSignal = !!r.isFavorable || (r.matchupEdge != null && r.matchupEdge >= 64) || !!r.isOnFire || hasNearHR(r.id);
    return hasSignal ? 'value' : 'longshot';
  }
  var HR_TIER_META = {
    elite:          { label: 'Elite',           icon: '💎', sub: 'Real predicted HR probability of 25% or higher' },
    highConfidence: { label: 'High Confidence', icon: '🎯', sub: '20–24.9% predicted HR probability' },
    value:          { label: 'Value',           icon: '🔍', sub: 'Lower predicted probability, but a real corroborating signal — favorable matchup, strong Matchup Edge, hot streak, or warning-track power' },
    longshot:        { label: 'Longshot',        icon: '🎲', sub: 'Still clears the board’s real inclusion bar, just the lowest predicted-probability tier' },
  };
  var HR_TIER_ORDER = ['elite', 'highConfidence', 'value', 'longshot'];
  // Groups already-sorted rows into tier sections, preserving each row's
  // existing relative order within its tier (whatever the current sort
  // column already produced) -- tiering never re-sorts, it only partitions.
  function hrTierSectionsHTML(arr, cardHTMLByIdx) {
    var byTier = { elite: [], highConfidence: [], value: [], longshot: [] };
    arr.forEach(function(r, i) { byTier[hrTierFor(r)].push(cardHTMLByIdx[i]); });
    return HR_TIER_ORDER.map(function(key) {
      var cards = byTier[key];
      if (!cards.length) return '';
      var meta = HR_TIER_META[key];
      return '<div class="dr1027-tier-section" data-tier="' + key + '">' +
        '<div class="dr1027-tier-head"><span class="dr1027-tier-icon">' + meta.icon + '</span>' +
        '<span class="dr1027-tier-title">' + meta.label + '</span>' +
        '<span class="dr1027-tier-count">' + cards.length + '</span>' +
        '<span class="dr1027-tier-sub">' + meta.sub + '</span></div>' +
        '<div class="dr1027-hr-card-list">' + cards.join('') + '</div></div>';
    }).join('');
  }
